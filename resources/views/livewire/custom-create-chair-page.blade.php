    ...
    <section class="mt-8">
        ...
        <div class="m-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                @foreach ($x3d_chair->model1_filepath as $item_model1_key => $item_model1)
                    <div class="card bg-white border rounded-lg shadow-md p-4 hover:shadow-lg cursor-pointer transition-transform transform-gpu hover:scale-105"
                        onclick="selectPart('{{ $item_model1 }}', 'model1', {{ $x3d_chair->price1[$item_model1_key]['price1'] ?? 'null' }})"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                @foreach ($x3d_chair->model2_filepath as $item_model2_key => $item_model2)
                    <div class="card bg-white border rounded-lg shadow-md p-4 hover:shadow-lg cursor-pointer transition-transform transform-gpu hover:scale-105"
                        onclick="selectPart('{{ $item_model2 }}', 'model2', {{ $x3d_chair->price2[$item_model2_key]['price2'] ?? 'null' }})"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                @foreach ($x3d_chair->model3_filepath as $item_model3_key => $item_model3)
                    <div class="card bg-white border rounded-lg shadow-md p-4 hover:shadow-lg cursor-pointer transition-transform transform-gpu hover:scale-105"
                        onclick="selectPart('{{ $item_model3 }}', 'model3', {{ $x3d_chair->price3[$item_model3_key]['price3'] ?? 'null' }})"
        ...
        <script>
            ...
            function addToCart() {
                ...
                var uniqueId = 'model' + 'chair' + ((Math.floor(Math.random() * 99) + 1) * 100);
                var uniqueName = 'chair-' + 'custom-' + ((Math.floor(Math.random() * 99) + 1) * 100);
